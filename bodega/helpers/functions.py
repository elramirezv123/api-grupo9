...
def send_product(productId, oc, address, price):
    ...
    headers["Authorization"]='INTEGRACION grupo9:{}'.format(hash)
    body = {"productoId": productId, "oc": oc, "direccion": address, "precio": int(price)}
...
def move_product_to_another_group(productId, almacenId):
    ...
    headers["Authorization"]='INTEGRACION grupo9:{}'.format(hash)
    body = {"productoId": productId, "almacenId": almacenId}
    response = requests.post(apiURL + "moveStockBodega", headers=headers, data=body)
...
def make_a_product(sku, quantity):
    ...
    response = request.put(apiURL + "fabrica/fabricarSinPago", headers=headers, data=body)
    return response
